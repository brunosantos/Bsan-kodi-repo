...
class Player(xbmc.Player):
    def __init__(self, url, idFilme, pastaData, temporada, episodio, nome, ano, logo):
        ...
        if self.temporada != 0 and self.episodio != 0:
            self.pastaVideo = os.path.join(self.pastaData, 'tracker',
                                           str(self.idFilme) + 'S' + str(self.temporada) + 'x' + str(
                                               self.episodio) + '.tugaio')
        else:
    ...
    def onPlayBackStarted(self):
        ...
        if xbmcvfs.exists(self.pastaVideo):
            ...
            dialog = xbmcgui.Dialog().yesno('Tuga.io', u'Já começaste a ver antes.',
                                            'Continuas a partir de %s?' % (tempoAux), '', 'Não', 'Sim')
            if dialog:
    ...
    def adicionarVistoBiblioteca(self):
        ...
        if int(self.temporada) != 0 and int(self.episodio) != 0:
            ficheiro = os.path.join(pastaVisto, str(self.idFilme) + 'S' + str(self.temporada) + 'x' + str(
                self.episodio) + '.tugaio')
        else:
        ...
        if not os.path.exists(ficheiro):
            ...
            try:
                ...
                if int(self.temporada) != 0 and int(self.episodio) != 0:
                    ...
                    dados = xbmc.executeJSONRPC(
                        '{"jsonrpc": "2.0", "method": "VideoLibrary.GetEpisodes", "params": {"filter":{"and": [{"field": "season", "operator": "is", "value": "%s"}, {"field": "episode", "operator": "is", "value": "%s"}]}, "properties": ["imdbnumber", "title", "year"]}, "id": 1}' % (
                    ...
                    dados = xbmc.executeJSONRPC(
                        ...
                        self.ano, str(int(self.ano) + 1), str(int(self.ano) - 1)))
            ...
            xbmc.executebuiltin(
                "XBMC.Notification(Tuga.io," + "Marcado como visto" + "," + "6000" + "," + self.logo + ")")
