...
class TVDB:
    ...
    def getSerieInfo(self, idIMDb):
        url = self.abrir_url(
            'http://www.thetvdb.com/api/GetSeriesByRemoteID.php?imdbid=' + idIMDb + '&language=' + self.lingua)
        soup = BeautifulSoup(url)
    ...
    def getSeasonEpisodio(self, idIMDb, season, episodio):
        ...
        url = self.abrir_url('http://thetvdb.com/api/' + self.api_key + '/series/' + str(serieId) + '/default/' + str(
            season) + '/' + str(episodio) + '/' + self.lingua + '.xml')
        soup = BeautifulSoup(url)
