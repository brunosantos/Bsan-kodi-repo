...
import urllib, urllib2, re, xbmcplugin, xbmcgui, xbmcvfs, xbmc, xbmcaddon, HTMLParser, json, os, time, datetime, \
    binascii, traceback, cookielib, base64
...
def Menu_principal():
    ...
    if login_sucessful == True:
        ...
        addDir_reg_menu('Favoritos', 'http://ratotv.net/favorites/page/1/', 15, artfolder + 'favoritos.jpg', True)
        ...
        addDir_reg_menu('Pedidos', "http://ratotv.net/requests/page/1/", 33, artfolder + 'contactar.jpg', True)
...
def Menu_principal_series():
    addDir_reg_menu('Todas as séries', base_url + 'index.php?cstart=1&do=cat&category=tvshows', 2,
                    artfolder + 'series.jpg', True)
    ...
    addDir_reg_menu('Séries a seguir', base_url + 'index.php?cstart=1&do=cat&category=tvshows', 26,
                    artfolder + 'series-a-seguir.jpg', True)
    addDir_reg_menu('Séries subscritas', base_url + 'index.php?cstart=1&do=cat&category=tvshows', 45,
                    artfolder + 'series-subscritas.jpg', True)
    menu_view()


def Menu_principal_animes():
    addDir_reg_menu('Todas as animes', base_url + 'index.php?cstart=1&do=cat&category=animes', 2,
                    artfolder + 'animes.jpg', True)
    ...
    addDir_reg_menu('', '', '', addonfolder + 'logo.png', False)
    addDir_reg_menu('Animes a seguir', base_url + 'index.php?cstart=1&do=cat&category=animes', 26,
                    artfolder + 'animes-a-seguir.jpg', True)
    addDir_reg_menu('Animes subscritas', base_url + 'index.php?cstart=1&do=cat&category=animes', 45,
                    artfolder + 'animes-subscritas.jpg', True)
    menu_view()
...
def Menu_categorias_filmes():
    ...
    match = re.findall(
        '<li class="dropdown"><a href="#"><img src="/templates/ratotvv2/dleimages/ico-4.png" />Género <span>v</span></a>(.*?)</div>\s+</ul>\s+</li>',
...
def menu_ano():
    addDir_reg_menu('[B][COLOR green]Introduzir ano manualmente[/B][/COLOR]', 'url', 41, artfolder + 'pesquisa.jpg',
                    True)
    for i in reversed(xrange(1900, 2015)): addDir_reg_menu(str(i), base_url + 'tags/' + str(i) + '/page/1/', 16,
                                                           artfolder + 'pesquisa.jpg', True)
...
def get_host_options(url, sources=None, with_progress=True):
    ...
    try:
        options = get_options(url, selfAddon.getSetting("login_name"), selfAddon.getSetting('login_password'), sources,
                              update_progress)
    except LoginError:
        if with_progress:
            progresso.close()
        ok = mensagemok('RatoTV',
                        'Não foi possível abrir a página. Tente novamente \n ou contacte um dos administradores do site.')
        sys.exit(0)
    else:
        if with_progress:
            progresso.close()
...
def player_rato(video, subs, name, url, iconimage, infolabels, season, episode):
    ...
    if subs and selfAddon.getSetting('subtitles-active') == 'true': player.setSubtitles(
        urllib.quote(subs, safe=":/"));print 'meti legendas', subs
    ...
    def __init__(self, url, season, episode):
        ...
        if selfAddon.getSetting('track_player') == 'true':
            try:
                ...
                self.id_rato = None
            if not xbmcvfs.exists(os.path.join(datapath, 'trackplayer')): xbmcvfs.mkdir(
                os.path.join(datapath, 'trackplayer'))
            if self.season and self.episode:
                self.tipo = 'tvshow'
                if self.id_rato:
                    self.filemedia = os.path.join(datapath, 'trackplayer',
                                                  str(self.id_rato) + 'S' + str(self.season) + 'E' + str(
                                                      self.episode) + '.txt')
                else:
                    self.filemedia = None
            else:
        ...
        try:
            ...
            trakt_instalado = False
        if trakt_instalado == True:
            save(os.path.join(datapath, 'trakt.txt'), trakt_addon.getSetting('rate_movie'))
            if trakt_addon.getSetting('rate_movie') == 'true': xbmcaddon.Addon(id='script.trakt').setSetting(
                'rate_movie', "false")
    ...
    def onPlayBackStarted(self):
        ...
        if selfAddon.getSetting('track_player') == 'true':
            if xbmcvfs.exists(self.filemedia):
                ...
                opcao = xbmcgui.Dialog().yesno("RatoTv", 'Existe um registo de visualização anterior.',
                                               'Continuar a partir de ' + ' %s?' % (format_time(float(tracker))), '',
                                               'Não', 'Sim')
...
def filmes_homepage(name, url):
    try:
        ...
        ok = mensagemok('RatoTV',
                        'Não foi possível abrir a página. Tente novamente ou contacte um dos administradores do site.');
        sys.exit(
            0)
    ...
    if html_source:
        match = re.compile('<img src="(.+?)" alt=".+?"/><span>(.+?)</span><a href="(.+?)"').findall(
            html_source_trunk[0])
        totalit = len(match)
        ...
        for img, titulo, url in match:
            ...
            if xbmcvfs.exists(txt_file):
                ...
                try:
                    ...
                    traceback.print_exc()
            i += 1
            addDir_filme(name + ' (' + str(infolabels["Year"]) + ')', url, mode, iconimage, infolabels, fanart, totalit,
                         pasta, filme_ou_serie, HD, favorito)
        progresso.close()
...
def obter_info_url(url, salvar=False):
    ...
    if html_source_trunk:
        infolabels, name, url, iconimage, fanart, filme_ou_serie, HD, favorito = rato_tv_get_media_info(
            html_source_trunk[0])
        if salvar:
            id_rato = re.compile('.*/(.+?)-.+?html').findall(url)
            userdata_folder = os.path.join(datapath, "media_database")
            ...
            txt_file = os.path.join(userdata_folder, id_rato[0] + '.txt')
            save(txt_file, name + '|' + url + '|' + str(infolabels) + '|' + iconimage + '|' + urllib.quote(
                fanart) + '|' + filme_ou_serie + '|' + str(HD) + '|' + str(favorito))
    ...
    for html_trunk in html_source_trunk:
        try:
            ...
            if filme_ou_serie == 'movie':
                addDir_filme(name + ' (' + infolabels['Year'] + ')', url, 3, iconimage, infolabels, fanart, totalit,
                             False, 'movie', HD, favorito)
            elif filme_ou_serie == 'tvshow':
                addDir_filme(name + ' (' + infolabels['Year'] + ')', url, 10, iconimage, infolabels, fanart, totalit,
                             True, 'tvshow', HD, favorito)
        except:
            pass
    try:
        addDir_reg_menu('[COLOR green]Pag (' + current_page[0] + '/' + total_paginas[0] + ') | Próxima >>>[/COLOR]',
                        pag_seguinte[0].replace('amp;', ''), mode, artfolder + 'seta.jpg', True)
    except:
        try:
            addDir_reg_menu(
                '[COLOR green]Pag (' + current_page[0] + '/' + total_paginas[0][0] + ') | Próxima >>>[/COLOR]',
                pag_seguinte[0].replace('amp;', ''), mode, artfolder + 'seta.jpg', True)
        except:
            pass
    moviesandseries_view()
...
def list_wachlist(url, mode):
    ...
    for html_trunk in html_source_trunk:
        try:
            infolabels, name, url, iconimage, fanart, filme_ou_serie, HD, favorito = rato_tv_get_media_info(html_trunk)
            addDir_filme(name + ' (' + infolabels['Year'] + ')', url, 3, iconimage, infolabels, fanart, totalit, False,
                         'movie', HD, favorito, watched=True)
        except:
            pass
    try:
        addDir_reg_menu('[COLOR green]Pag (' + current_page[0] + '/' + total_paginas[0] + ') | Próxima >>>[/COLOR]',
...
def listar_pesquisa(url, retorna=False):
    ...
    encode = True
    try:
        html_source = post_page(urllib.unquote(url), selfAddon.getSetting('login_name'),
                                selfAddon.getSetting('login_password'))
        html_source_trunk = re.findall('<div class="shortpost">(.*?)<\/div>\n<\/div>\n<\/div>', html_source, re.DOTALL)
    ...
    if totalit != 0:
        for html_trunk in html_source_trunk:
            try:
                ...
                if filme_ou_serie == 'movie':
                    if retorna != "values":
                        addDir_filme(name + ' (' + str(infolabels['Year']) + ')', url, 3, iconimage, infolabels, fanart,
                                     totalit, False, 'movie', HD, favorito)
                    else:
                        resultado_pesquisa = url
                elif filme_ou_serie == 'tvshow':
                    if retorna != "values":
                        addDir_filme(name + ' (' + str(infolabels['Year']) + ')', url, 10, iconimage, infolabels,
                                     fanart, totalit, True, 'tvshow', HD, favorito)
                    else:
                        resultado_pesquisa = url
            except:
                pass
        if not retorna:
            try:
                ...
                if encode:
                    pagina_seguinte = re.sub('search_start=[0-9]+', 'search_start=' + pag_seguinte[0],
                                             URLpesquisa).replace('amp;', '')
                else:
                    pagina_seguinte = URLpesquisa.replace('/' + current_page[0] + '/',
                                                          '/' + str(int(current_page[0]) + 1) + '/')
                if int(pag_seguinte) > int(current_page[0]):
                    addDir_reg_menu(
                        '[COLOR green]Pag (' + current_page[0] + '/' + total_paginas + ') | Próxima >>>[/COLOR]',
                        pagina_seguinte, 16, artfolder + 'seta.jpg', True)
                else:
                    pass
            except:
                pass
...
def rato_tv_get_media_info(html_trunk):
    data_dict = dict(
        [('code', ''), ('Count', ''), ('Title', ''), ('Year', ''), ('Rating', ''), ('Genre', ''), ('Director', ''),
         ('Cast', list()), ('Plot', ''), ('Trailer', '')])
    ...
    for url_newsid, iconimage, name in match:
        ...
        if iconimage.find('https://') == -1:
            thumbnail = base_url + iconimage
        else:
            thumbnail = iconimage
    ...
    if match:
        # print "Found rating"
        for score in match: data_dict['Rating'] = float(
            score.replace(',', '.').replace('<div class="rating1">', '').replace('<span>', '').replace('</span>',
                                                                                                       '').replace(
                '</div>', ''))
    else:
    ...
    match = re.compile('<a href="http://ratotv.net/.+?">(.+?)</a></li>').findall(html_trunk)
    # print "Filme ou Série",match
    for categoria in match:
        if categoria == 'Filmes':
            filme_ou_serie = 'movie'
        elif categoria == 'Séries' or categoria == 'Animes':
            filme_ou_serie = 'tvshow'
    ...
    match = re.compile('<a href="http://ratotv.net/xfsearch/.D/">(.D)</a>').findall(html_trunk)
    HD = None
    if match:
        if match[0] == 'HD':
            HD = True
            ...
            try:
                for key in json_code:
                    if str(key['season']) == str(season):
                        try:
                            iconimage = key['images']['poster']
                        except:
                            pass
                addDir_temporada("[B][COLOR white]Temporada[/B][/COLOR] " + str(season), url,
                                 str(serie_dict_temporadas), 39, iconimage, True, fanart)
            except:
                addDir_temporada("[B][COLOR white]Temporada[/B][/COLOR] " + str(season), url,
                                 str(serie_dict_temporadas), 39, iconimage, True, fanart)
        else:
            addDir_temporada("[B][COLOR white]Temporada[/B][/COLOR] " + str(season), url, str(serie_dict_temporadas),
                             39, iconimage, True, fanart)
    series_view()
...
def downloader_rato(video, headers, subs, name, url, iconimage, infolabels, season, episode):
    ...
    try:
        if episode:
            ...
            u = urllib2.urlopen(request, timeout=1000)
            if os.path.exists(ADDON.getSetting('folder') + 'filmes/'):
                pass
            else:
...
def check_login():
    if selfAddon.getSetting('login_name') == '' or selfAddon.getSetting('login_password') == '':
        ...
        try:
            html_source = post_page(base_url, selfAddon.getSetting('login_name'),
                                    selfAddon.getSetting('login_password'))
            ...
            return resultado
        if match == []:
            match = re.compile('href="http://ratotv.net/user/(.+?)/">Perfil').findall(html_source)
            if match == []:
                resultado = False
                ...
                resultado = True
                xbmc.executebuiltin("XBMC.Notification(RatoTv," + selfAddon.getSetting(
                    'login_name') + " -Sessão iniciada!,'10000'," + addonfolder + "/icon.png)")
                return resultado
        else:
            resultado = True
            xbmc.executebuiltin("XBMC.Notification(RatoTv," + selfAddon.getSetting(
...
def comment(url):
    ...
    if post_id == [] or len(post_id) != 1:
        mensagemok('RatoTV', 'Ocorreu um erro a determinar correctamente o ID do post. Informe os administradores.')
    else:
        keyb = xbmc.Keyboard('', 'Escreva o parâmetro de pesquisa')
        keyb.doModal()
        if (keyb.isConfirmed()):
            ...
            if comentario == '':
                return mensagemok('RatoTV', 'Não escreveu nenhum comentário.')
            else:
                comentario += "\n.:.Comentário enviado do XBMC.:."
                mydata = [('login_name', selfAddon.getSetting('login_name')),
                          ('login_password', selfAddon.getSetting('login_password')), ('login', 'submit'),
                          ('subaction', 'addcomment'), ('name', selfAddon.getSetting('login_name')),
                          ('post_id', post_id[0]), ('comments', comentario)]
...
def ler_comentarios(url, todos_os_comentarios):
    try:
        ...
        match = []
    if match == []:
        return mensagemok('RatoTV', 'Não existem comentários ao filme.')
    else:
        for comentario in match:
            autor = re.compile('href="http://ratotv.net/user/.+?">(.+?)</a>').findall(comentario)
            ...
            quote = re.compile('<!--QuoteBegin<!--QuoteEBegin-->(.+?)<!--QuoteEnd--></div><!--QuoteEEnd-->').findall(
                texto)
            for quo in quote: texto = texto.replace(quo, '')
            ...
            for quo in quote: texto = texto.replace(quo, '')
            texto = texto.replace('<pre>', '').replace('</pre>', '').replace('</span>', '').replace('<!--smile:',
                                                                                                    '').replace(
                '<!--/smile-->', '').replace('<!--QuoteBegin', '').replace('<!--QuoteEBegin-->', '').replace(
                '<!--QuoteEnd-->', '').replace('</div><!--QuoteEEnd-->', '').replace('<b>', '').replace('</b>',
                                                                                                        '').replace(
                '&eacute;', '').replace('&nbsp;', '').replace('<p>', '').replace('</p>', '').replace('&atilde;',
                                                                                                     'ã').replace(
                '&aacute;', 'á').replace('&ccedil;', 'ç')
            todos_os_comentarios = todos_os_comentarios + '[B]' + autor[0] + ' em ' + data[
                0] + '[/B]\n' + texto + '\n-------------------------------------------------------\n\n'
    pag_seguinte = re.compile('href="(.+?)" onclick=".+?">Seguinte</a>').findall(html_source)
    try:
        ler_comentarios(pag_seguinte[0], todos_os_comentarios)
    except:
        janela_lateral('Comentários: ', todos_os_comentarios)
...
def votar(url, voto):
        ...
        if trakt_addon.getSetting('rate_movie') == 'true':
            ...
            if trakt_login_check == True:
                ...
                try:
                    ...
                    pass
                if filme_ou_serie == 'movie':
                    url_json_post = "http://api.trakt.tv/rate/movie/353f223c2afc3c2050fcb810810fdb49"
                elif filme_ou_serie == 'tvshow':
                    url_json_post = "http://api.trakt.tv/rate/show/353f223c2afc3c2050fcb810810fdb49"
                try:
                    json_post(data, url_json_post)
                except:
                    print "Não conseguiu enviar rate para o trakt"
            else:
                pass
        else:
            pass
    try:
        ...
        return xbmc.executebuiltin(
            "XBMC.Notification(RatoTv,Obrigado pelo seu voto!,'10000'," + addonfolder + "/icon.png)")
    except:
...
def reportar(url):
    id_ratotv = re.compile('.*/(.+?)-.+?html').findall(url)
    if id_ratotv == [] or len(id_ratotv) != 1:
        mensagemok('RatoTV', 'Ocorreu um erro a determinar correctamente o ID do post. Informe os administradores.')
    else:
        yes = xbmcgui.Dialog().yesno("RatoTv", 'Pretende escolher um dos problemas comuns (pré-definidos)?')
        if yes:
            label = ['Episódios em falta', 'Qualidade fraca', 'Filme incompleto', 'Sem som', 'Filme não toca',
                     'Qualidade não corresponde ao anunciado', 'Legendas não estão sincronizadas', 'Sem legendas'];
            text = ['A série apresenta episódios em falta', 'O filme/série tem qualidade fraca',
                    'Não consegui reproduzir o filme até ao fim. Encontra-se incompleto', 'O filme/série não tem som',
                    'Não consegui reproduzir o filme', 'A qualidade do filme/série não corresponde à label anunciada.',
                    'As legendas do filme/série não estão sincronizadas', 'Existem vídeos sem legendas']
            ...
            try:
                if choose > -1:
                    try:
                        ...
                        mydata = [('login_name', selfAddon.getSetting('login_name')),
                                  ('login_password', selfAddon.getSetting('login_password')), ('login', 'submit'),
                                  ('id', str(id_ratotv[0])), ('action', 'news'),
                                  ('text', urllib.unquote_plus(text[choose]))]
            ...
            if (keyb.isConfirmed()):
                comentario = keyb.getText()
                if comentario == '':
                    return mensagemok('', 'Não escreveu texto.')
                else:
                    try:
                        url = base_url + 'engine/ajax/complaint.php'
                        mydata = [('login_name', selfAddon.getSetting('login_name')),
                                  ('login_password', selfAddon.getSetting('login_password')), ('login', 'submit'),
                                  ('id', str(id_ratotv[0])), ('action', 'news'),
                                  ('text', urllib.unquote_plus(comentario))]
                        post_page_free(url, mydata)
...
def filmes_watchlist(name):
    ...
    if movies_watch:
        ...
        if name == 'actualizarlib': xbmc.executebuiltin(
            "XBMC.UpdateLibrary(video," + os.path.join(moviesFolder, '') + ")")
    return
...
def filmes_collection_trakt(name):
    ...
    if movies_watch:
        ...
        for title, year, imdb_id in movies_watch:
            ...
            url_rato = listar_pesquisa(urllib.quote_plus(url_pesquisa), 'values')
            if url_rato: adicionar_filme_biblioteca(title, url_rato, '', False, True)
            progresso.update(int(((i)) / (total_items) * 100), 'A procurar filme do trakt no RatoTV...',
                             title + ' (' + year + ')')
...
def series_collection_trakt(name):
    ...
    if movies_watch:
        ...
        for title, year, imdb_id in movies_watch:
            ...
            url_rato = listar_pesquisa(urllib.quote_plus(url_pesquisa), 'values')
            if url_rato: subscrever_serie(title, url_rato, '', daemon=True)
            progresso.update(int(((i)) / (total_items) * 100), 'A procurar serie do trakt no RatoTV...',
                             title + ' (' + year + ')')
        progresso.update(100, "Terminado!")
        progresso.close()
...
def filmes_trending():
    ...
    for title, year, imdb_id in movies_trend:
        ...
        progresso.update(int(((i)) / (total_items) * 100), 'A procurar trending do trakt no RatoTV...',
                         title + ' (' + year + ')')
    progresso.update(100, "Terminado!")
    progresso.close()
    moviesandseries_view()
...
def proximo_episodio(url):
    ...
    while i < len(episodios_dict):
        ...
        i += 1
    if status == 'Ended':
        mensagemok('RatoTV', 'A série está completa. Não existem mais episódios ', 'para esta série.')
    else:
        try:
            next_episode = i + 1
            channel = data["network"]
            season = len(data["seasons"])
            mensagemok('RatoTV', 'Próximo episódio: S' + str(season) + 'E' + str(i + 1),
                       'A exibir no canal ' + channel + ' no dia: ' + datetime.datetime.fromtimestamp(
                           int(episodios_dict[i + 1])).strftime('%Y-%m-%d'))
        except:
            mensagemok('RatoTV', 'Não existe informação para o próximo episódio')
...
def estatisticas_trakt(url):
    ...
    if html_source:
        try:
            infolabels, name, url2, iconimage2, fanart2, filme_ou_serie, HD2, favorito2 = rato_tv_get_media_info(
                html_source)
        except:
            pass
        if infolabels['code']:
            if filme_ou_serie == 'movie':
                url_api_trakt_now = 'http://api.trakt.tv/movie/watchingnow.json/' + trakt_api().api_key + '/' + \
                                    infolabels['code']
                url_api_trakt = 'http://api.trakt.tv/movie/summary.json/' + trakt_api().api_key + '/' + infolabels[
                    'code'] + '/extended'
            elif filme_ou_serie == 'tvshow':
                if season and episode:
                    url_api_trakt_now = 'http://api.trakt.tv/show/episode/watchingnow.json/' + trakt_api().api_key + '/' + \
                                        infolabels['code'] + '/' + season + '/' + episode
                    url_api_trakt = 'http://api.trakt.tv/show/episode/summary.json/' + trakt_api().api_key + '/' + \
                                    infolabels['code'] + '/' + season + '/' + episode
                else:
                    url_api_trakt_now = 'http://api.trakt.tv/show/watchingnow.json/' + trakt_api().api_key + '/' + \
                                        infolabels['code']
                    url_api_trakt = 'http://api.trakt.tv/show/summary.json/' + trakt_api().api_key + '/' + infolabels[
                        'code'] + '/extended'
            else:
                pass
            texto = ''
            try:
                ...
                if len(data) > 0:
                    ...
                    texto += 'Ninguém a assistir neste momento \n\n'
            except:
                pass
            data = json_get(url_api_trakt)
            try:
                texto += 'Já viram: ' + str(data['stats']['watchers']) + ' pessoas\n\n'
            except:
                pass
            try:
                texto += 'Visto: ' + str(data['stats']['plays']) + ' vezes\n\n'
            except:
                pass
            try:
                texto += str(data['ratings']['loved']) + ' gostam (' + str(data['ratings']['percentage']) + '%)\n\n'
            except:
                pass
            try:
                texto += str(data['ratings']['hated']) + ' não gostam\n\n'
            except:
                pass
            try:
                texto += 'Total de votos: ' + str(data['ratings']['votes']) + '\n\n'
            except:
                pass
            try:
                texto += "Check-in's: " + str(data['stats']['checkins']) + '\n\n'
            except:
                pass
            try:
                texto += "Na colecção de " + str(data['stats']['collection']) + ' utilizadores' + '\n\n'
            except:
                pass
            try:
                texto += 'Já viram: ' + str(data['episode']['stats']['watchers']) + ' pessoas\n\n'
            except:
                pass
            try:
                texto += 'Visto: ' + str(data['episode']['stats']['plays']) + ' vezes\n\n'
            except:
                pass
            try:
                texto += str(data['episode']['ratings']['loved']) + ' gostam (' + str(
                    data['episode']['ratings']['percentage']) + '%)\n\n'
            except:
                pass
            try:
                texto += str(data['episode']['ratings']['hated']) + ' não gostam\n\n'
            except:
                pass
            try:
                texto += 'Total de votos: ' + str(data['episode']['ratings']['votes']) + '\n\n'
            except:
                pass
            try:
                texto += "Check-in's: " + str(data['episode']['stats']['checkins']) + '\n\n'
            except:
                pass
            try:
                texto += "Na colecção de " + str(data['episode']['stats']['collection']) + ' utilizadores' + '\n\n'
            except:
                pass
...
def janela2qualidades(prioridade=None):
    if selfAddon.getSetting('fonte-auto') == "false":
        ...
        if prioridade == "":
            ...
            if selfAddon.getSetting('host1') == "Opção 2" or selfAddon.getSetting('host1') == "Opção 3": save(
                datapath + "option.txt", "2")
            return
        if prioridade == "1":
            try:
                save(datapath + "option.txt", "")
            ...
            if selfAddon.getSetting('host2') == "Opção 2" or selfAddon.getSetting('host2') == "Opção 3": save(
                datapath + "option.txt", "2")
            return
        if prioridade == "2":
            try:
            ...
            if selfAddon.getSetting('host3') == "Opção 2" or selfAddon.getSetting('host3') == "Opção 3": save(
                datapath + "option.txt", "2")
            return
...
class JANELA_VOTO(xbmcgui.WindowXMLDialog):
    ...
    focus_labels = {10030: 1314, 10031: 1315, 11030: 1315, 11031: 1316, 11032: 1317, 11033: 1318, 11034: 1319,
                    11035: 1320, 11036: 1321, 11037: 1322, 11038: 1323, 11039: 1314}
...
def menu_pedidos(url):
    addDir_reg_menu("[B][COLOR green]Pedir outro filme/série?[/COLOR][/B]", "rato", 13, artfolder + 'contactar.jpg',
                    True, fanart=fanart_rato_tv)
    pag_actual = url.split('/')[-2]
    if int(pag_actual) == 1 and selfAddon.getSetting('mensagem-pedidos') == "true":
        yes = xbmcgui.Dialog().yesno('RatoTV', 'Clique nos filmes/séries para registar o pedido.',
                                     "Pode também pedir outros filmes/séries.", "Continuar a apresentar esta mensagem?",
                                     'Não', 'Sim')
        if yes == 0:
    ...
    for trunk in html_source_trunk:
        ...
        img = img_titulo[0][0].replace("http//www.ratotv.net/", base_url)
        try:
            addDir_reg_menu(img_titulo[0][1] + "[COLOR green] (" + pedidos[0] + " pedidos)[/COLOR]", id_pedido[0], 34,
                            img, False, fanart=fanart_rato_tv)
        except:
            pass
    match = re.compile('<a href="http://ratotv.net/requests/page/.+?/">(.+?)</a>').findall(html_source)
    pag_total = match[-1]
    try:
        addDir_reg_menu(
            "[COLOR green]Página " + str(pag_actual) + "/" + str(pag_total) + " |[B] Seguinte >>[/COLOR][/B]",
            'http://ratotv.net/requests/page/' + str(int(pag_actual) + 1) + '/', 33, artfolder + 'seta.jpg', True,
            fanart=fanart_rato_tv)
    except:
        pass
    pedidos_view()
...
def pedir_serie_menu():
    yes = xbmcgui.Dialog().yesno('RatoTV', '1) Procurar por nome de filme/série.', "2) Introduzir id do imdb.",
                                 "Que opção deseja?", '2)', '1)')
    if yes == 0:
        ...
        if len(data) >= 1:
            addDir_reg_menu("[B][COLOR green]Filmes:[/COLOR][/B]", "", 13, artfolder + 'filmes.jpg', False,
                            fanart=fanart_rato_tv)
        ...
        if len(data) >= 1:
            addDir_reg_menu("[B][COLOR green]Séries:[/COLOR][/B]", "", 13, artfolder + 'series.jpg', False,
                            fanart=fanart_rato_tv)
            for i in xrange(0, len(data)):
                imdb_id = data[i]["imdb_id"]
                titulo = data[i]["title"]
...
def pedir_imdb(imdb):
    ...
    source = post_page_free("http://ratotv.net/engine/ajax/mws-film.ajax.php", mydata)
    if '"html":' in source:
        xbmc.executebuiltin(
            "XBMC.Notification(RatoTV," + "Pedido efectuado com sucesso." + "," + "6000" + "," + addonfolder + "/icon.png)")
    elif '"Ja fizeste este pedido!"' in source:
        mensagemok('RatoTV', 'Lamentamos mas já fez o pedido anteriormente.')
    else:
        mensagemok('RatoTV', 'Não foi possível encontrar o filme/série.', 'Verifique novamente.')
...
def mensagens_conta():
    try:
        html_source = post_page("http://ratotv.net/index.php?do=pm&folder=inbox", selfAddon.getSetting('login_name'),
                                selfAddon.getSetting('login_password'))
    except:
        html_source = ""
    ...
    try:
        addDir_reg_menu('Mensagens privadas [COLOR green][ [B]' + str(
            num_msg_unread) + '[/B] não lida(s) [/COLOR]|[COLOR yellow] ' + cheia[
                            0] + ' cheia [/COLOR][COLOR green]][/COLOR]', 'url', 36,
                        artfolder + 'mensagens-privadas.jpg', True)
    except:
        pass
...
def listar_pms():
    ...
    for endereco_msg, sender in match:
        if "<b>" in sender:
            addDir_mensagem('[B]' + sender.replace('<b>', '').replace('</b>', '') + '[/B]', endereco_msg, 37,
                            artfolder + 'contactar.jpg', False, "nlida")
        else:
...
def ler_pm(url):
    try:
        html_source = post_page(url.replace("amp;", ""), selfAddon.getSetting('login_name'),
                                selfAddon.getSetting('login_password'))
    except:
        html_source = ""
...
def apagar_pm(url):
    ...
    try:
        html_source = post_page("http://ratotv.net/index.php?do=pm&doaction=readpm&pmid=" + pm_id,
                                selfAddon.getSetting('login_name'), selfAddon.getSetting('login_password'))
    except:
        html_source
    if html_source:
        match = re.compile("javascript:confirmDelete\('(.+?)'\)\">Apagar</a>").findall(html_source)
        try:
            html_source = post_page(match[0].replace('amp;', ''), selfAddon.getSetting('login_name'),
                                    selfAddon.getSetting('login_password'))
            xbmc.executebuiltin(
                "XBMC.Notification(RatoTV," + "Mensagem eliminada com sucesso!" + "," + "6000" + "," + addonfolder + "/icon.png)")
            xbmc.executebuiltin("XBMC.Container.Refresh")
        except:
...
def add_to_favourites(url):
    ...
    xbmc.executebuiltin(
        "XBMC.Notification(RatoTV," + "adicionado aos favoritos" + "," + "6000" + "," + addonfolder + "/icon.png)")
    xbmc.executebuiltin("XBMC.Container.Refresh")
...
def remover_favoritos(url):
    ...
    html_source = post_page(postURL, selfAddon.getSetting('login_name'), selfAddon.getSetting('login_password'))
    xbmc.executebuiltin(
        "XBMC.Notification(" + "RatoTv" + "," + "Removido dos favoritos" + "," + "6000" + "," + addonfolder + "/icon.png)")
    xbmc.executebuiltin("XBMC.Container.Refresh")
...
def adicionar_seguir(url, name, iconimage):
    ...
    try:
        ...
        if text != '':
            ...
            xbmc.executebuiltin(
                "XBMC.Notification(RatoTV," + "Operação com sucesso!" + "," + "6000" + "," + addonfolder + "/icon.png)")
            xbmc.executebuiltin("XBMC.Container.Refresh")
        else:
            pass
    except:
...
def deixar_seguir(url):
    ...
    try:
        os.remove(filename)
        xbmc.executebuiltin(
            "XBMC.Notification(RatoTV," + "Série removida com sucesso!" + "," + "6000" + "," + addonfolder + "/icon.png)")
        xbmc.executebuiltin("XBMC.Container.Refresh")
    except:
...
def verificar_novos():
    ...
    if dircontents:
        ...
        while i < len(dircontents):
            ...
            text += texto
            progresso.update(int(((i + 1)) / (len(dircontents)) * 100),
                             "A verificar novos episódios nas séries seguidas", seriename)
            i += 1
        if text != '':
            return janela_lateral('Séries seguidas: ', text)
        else:
            return xbmc.executebuiltin(
                "XBMC.Notification(RatoTV," + "Não há novos episódios." + "," + "6000" + "," + addonfolder + "/icon.png)")
    else:
        pass
...
def verificar_novoepisodio_serie(txt):
    ...
    try:
        ...
        if last_season_num != match[0][3] or last_available_episode != match[0][4]:
            text += 'Novo episódio de ' + match[0][
                0] + ' disponível! Temporada: ' + last_season_num + ' Episódio: ' + last_available_episode + '\n\n'
            textnew = '|' + match[0][0] + '|' + match[0][1] + '|' + match[0][
                2] + '|' + last_season_num + '|' + last_available_episode + '|'
...
def adicionar_rato_visto(url, season=None, episode=None):
    try:
        ...
        if season and episode:
            ...
            post_page(base_url + "engine/ajax/watched.php?mov_id=%s&action=plus&skin=ratotvv2" % id_ratotv,
                      selfAddon.getSetting('login_name'), selfAddon.getSetting('login_password'))
    except:
        traceback.print_exc()
        ...
        xbmc.executebuiltin(
            "XBMC.Notification(RatoTV," + "Marcado como visto (site)" + "," + "6000" + "," + addonfolder + "/icon.png)")
        xbmc.executebuiltin("XBMC.Container.Refresh")
...
def remover_rato_visto(url, season=None, episode=None):
    try:
        ...
        if season and episode:
            ...
            post_page(base_url + "engine/ajax/watched.php?mov_id=%s&action=minus&skin=ratotvv2" % id_ratotv,
                      selfAddon.getSetting('login_name'), selfAddon.getSetting('login_password'))
    except:
        traceback.print_exc()
        ...
        xbmc.executebuiltin(
            "RatoTv," + "Marcado como não visto (site)" + "," + "6000" + "," + addonfolder + "/icon.png)")
        xbmc.executebuiltin("XBMC.Container.Refresh")
...
def adicionar_visto(url, season=None, episode=None):
        ...
        if trakt_login_check == True:
            if filme_ou_serie == 'movie':
                ...
                try:
                    ...
                    print "Não conseguiu marcar visto no trakt"
            elif filme_ou_serie == 'tvshow':
    ...
    try:
        ...
        pass
    if season and episode:
        NewVistoFile = os.path.join(vistospath, id_ratotv + '-S' + season + 'E' + episode + '.txt')
    else:
        NewVistoFile = os.path.join(vistospath, id_ratotv + '.txt')
        ...
        try:
            if not season and not episode:
                if xbmc.getCondVisibility('Library.HasContent(Movies)'):
                    html_source = post_page(url, selfAddon.getSetting('login_name'),
                                            selfAddon.getSetting('login_password'))
                    infolabels, name, url2, iconimage2, fanart2, filme_ou_serie, HD2, favorito2 = rato_tv_get_media_info(
                        html_source)
                    print "A verificar se filme existe na biblioteca"
                    meta = xbmc.executeJSONRPC(
                        '{"jsonrpc": "2.0", "method": "VideoLibrary.GetMovies", "params": {"filter":{"or": [{"field": "year", "operator": "is", "value": "%s"}, {"field": "year", "operator": "is", "value": "%s"}, {"field": "year", "operator": "is", "value": "%s"}]}, "properties" : ["file"]}, "id": 1}' % (
                            infolabels["Year"], str(int(infolabels["Year"]) + 1), str(int(infolabels["Year"]) - 1)))
                    ...
                    xbmc.executeJSONRPC(
                        '{"jsonrpc": "2.0", "method": "VideoLibrary.SetMovieDetails", "params": {"movieid" : %s, "playcount" : 1 }, "id": 1 }' % str(
                            meta['movieid']))
            else:
                if xbmc.getCondVisibility('Library.HasContent(TVShows)'):
                    ...
                    cleaned_title = re.sub('[^-a-zA-Z0-9_.()\\\/ ]+', '', originaltitle)
                    meta = xbmc.executeJSONRPC(
                        '{"jsonrpc": "2.0", "method": "VideoLibrary.GetEpisodes", "params": {"filter":{"and": [{"field": "season", "operator": "is", "value": "%s"}, {"field": "episode", "operator": "is", "value": "%s"}]}, "properties": ["title", "plot", "votes", "rating", "writer", "firstaired", "playcount", "runtime", "director", "productioncode", "season", "episode", "originaltitle", "showtitle", "lastplayed", "fanart", "thumbnail", "file", "resume", "tvshowid", "dateadded", "uniqueid"]}, "id": 1}' % (
                            season, episode))
                    meta = unicode(meta, 'utf-8', errors='ignore')
                    meta = json.loads(meta)
            ...
            pass
        xbmc.executebuiltin(
            "XBMC.Notification(RatoTV," + "Marcado como visto" + "," + "6000" + "," + addonfolder + "/icon.png)")
        xbmc.executebuiltin("XBMC.Container.Refresh")
    else:
...
def remover_visto(url, season=None, episode=None):
    ...
    if season and episode:
        ...
        NewVistoFile = os.path.join(vistospath, id_ratotv + '.txt')
    try:
        os.remove(NewVistoFile)
    except:
        mensagemok('RatoTV', 'Não foi possível marcar como não visto.')
...
def check_visto(url, season=None, episode=None):
    ...
    vistospath = os.path.join(datapath, 'Vistos')
    if season and episode:
        NewVistoFile = os.path.join(vistospath, id_ratotv + '-S' + str(season) + 'E' + str(episode) + '.txt')
    else:
        NewVistoFile = os.path.join(vistospath, id_ratotv + '.txt')
    if os.path.exists(NewVistoFile):
        return True
    else:
        return False
...
class trakt_api:
    ...
    def get_movie_watchlist(self):
        url_api = 'http://api.trakt.tv/user/watchlist/movies.json/' + self.api_key + '/' + selfAddon.getSetting(
            'trakt_login')
        try:
            data = json_get(url_api)
        except:
    ...
    def get_series_watchlist(self):
        url_api = 'http://api.trakt.tv/user/watchlist/shows.json/' + self.api_key + '/' + selfAddon.getSetting(
            'trakt_login')
        try:
            data = json_get(url_api)
        except:
    ...
    def get_movie_colection(self):
        url_api = 'http://api.trakt.tv/user/library/movies/collection.json/' + self.api_key + '/' + selfAddon.getSetting(
            'trakt_login')
        try:
            data = json_get(url_api)
        except:
    ...
    def get_shows_colection(self):
        url_api = 'http://api.trakt.tv/user/library/shows/collection.json/' + self.api_key + '/' + selfAddon.getSetting(
            'trakt_login')
        try:
            data = json_get(url_api)
        except:
...
class themoviedb_api:
    ...
    def fanart_and_id(self, movie_info_original_title, movie_info_year):
        url_tmdb = 'http://api.themoviedb.org/3/search/movie?api_key=' + self.api_key + '&query=' + urllib.quote_plus(
            movie_info_original_title) + '&year=' + movie_info_year
        try:
...
class thetvdb_api:
    def _id(self, series_name, year):
        ...
        try:
            ...
            html_source = ''
        id_and_year = re.findall('<seriesid>(.+?)</seriesid>.*?<FirstAired>(.+?)-.+?-.+?</FirstAired>', html_source,
                                 re.DOTALL)
        if id_and_year == []:
            ...
            for serieid, ano in id_and_year:
                if ano == year:
                    _id_init.append(serieid)
                else:
                    pass
            if _id_init == []:
                return id_and_year[0][0]
            else:
                return _id_init[0]

    def fanart(self, series_id):
        fanart_image = 'http://thetvdb.com/banners/fanart/original/' + series_id + '-1.jpg'
        if check_if_image_exists(fanart_image):
            fanart_image = fanart_rato_tv
        else:
            pass
        return fanart_image
...
class fanarttv_api:
    ...
    def fanart(self, series_id):
        ...
        if len(fanart_vector) >= 1:
            return fanart_vector[0]
        else:
            return ''
    ...
    def season_thumbs(self, temporada, series_id):
        try:
            url = 'http://api.fanart.tv/webservice/series/' + self.api_key + '/' + series_id + '/xml/'
            html_source = abrir_url(url)
            thumb_vector = re.compile(
                '<seasonthumb id=".+?" url="(.+?)" lang=".+?" likes=".+?" season="%s"/>' % temporada).findall(
                html_source)
            if len(thumb_vector) >= 1:
                return thumb_vector[0]
            else:
                return ''
        except:
            return ''
...
def handle_wait(time_to_wait, title, text, segunda=''):
    ...
    while secs < time_to_wait:
        ...
        if segunda == '':
            remaining_display = "Faltam " + secs_left + " segundos"
        else:
...
def adicionar_filme_biblioteca(name, url, iconimage, updatelibrary=True, fromTrakt=False):
    ...
    if selfAddon.getSetting('libraryfolder'):
        ...
        ok = mensagemok('RatoTV', 'Não definiu uma pasta para a biblioteca nas definições!',
                        'Por favor defina-a e tente novamente.')
        sys.exit(0)
    if not xbmcvfs.exists(moviesFolder): xbmcvfs.mkdir(moviesFolder)
    id_ratotv = re.compile('.*/(.+?)-.+?html').findall(url)[0]
    try:
        html_source = post_page(current_url, selfAddon.getSetting('login_name'), selfAddon.getSetting('login_password'))
    except:
        html_source = ''
    if html_source:
        ...
        if html_source_trunk:
            ...
            infolabels, name, url, iconimage, fanart, filme_ou_serie, HD, favorito = rato_tv_get_media_info(
                html_source_trunk[0])
            if fromTrakt:
                movie_folder = os.path.join(moviesFolder, nameTrakt + ' (' + str(infolabels["Year"]) + ')')
            else:
            ...
            if fromTrakt:
                strm_contents = 'plugin://plugin.video.ratotv/?url=' + url + '&mode=44&name=' + urllib.quote_plus(
                    nameTrakt)
            else:
                strm_contents = 'plugin://plugin.video.ratotv/?url=' + url + '&mode=44&name=' + urllib.quote_plus(
            ...
            if fromTrakt:
                movie_biblioteca_file = os.path.join(movie_folder, urllib.quote_plus(nameTrakt) + '.strm')
            else:
                movie_biblioteca_file = os.path.join(movie_folder,
                                                     urllib.quote_plus(infolabels['originaltitle']) + '.strm')
            save(movie_biblioteca_file, strm_contents)
            save(movie_database_file, "check")
            if updatelibrary:
                xbmc.executebuiltin(
                    "XBMC.Notification(RatoTv,Filme adicionado à biblioteca!,'10000'," + addonfolder + "/icon.png)")
    ...
    if cancelar:
        if name == "service":
            ...
            ok = mensagemok('RatoTV', 'Não tem séries subscritas',
                            'Subscreva novas séries ou desactive a procura de novas séries!')
            sys.exit(0)
    else:
...
def transferir_biblioteca_filmes(name, tipo=None):
    ...
    if comando == 'novos':
        if not xbmcvfs.exists(movie_database_folder):
            ok = mensagemok('RatoTV', 'Tem o serviço de procura de novos filmes activo.',
                            'Mas nao transferiu a biblioteca...', 'Saindo.')
            sys.exit(0)
    ...
    if html_source:
        if tipo == 'favorites':
            try:
                ...
                try:
                    ...
                    total_paginas = re.compile('.*/page/(.+?)/">(.+?)</a> ').findall(html_source)[0]
            ...
            current_page = int(current_page)
        html_source_trunk = re.findall('<div class="shortpost">(.*?)<\/div>\n<\/div>\n<\/div>', html_source, re.DOTALL)
        if comando == 'todos': progresso.create('RatoTV - Biblioteca XBMC',
                                                'A transferir biblioteca de Filmes...Aguarde...',
                                                'Página ' + str(current_page) + '/' + str(total_paginas))
        i = 0
        while int(current_page) < int(total_paginas):
            ...
            if current_page != 1:
                try:
                    html_source = post_page(pag_seguinte, selfAddon.getSetting('login_name'),
                                            selfAddon.getSetting('login_password'))
                except:
                    html_source = ''
                pag_seguinte = re.compile('<div class="next"><a href="(.+?)">').findall(html_source)[0]
            ...
            for trunk in html_source_trunk:
                ...
                cleaned_title = re.sub('[^-a-zA-Z0-9_.()\\\/ ]+', '', infolabels['originaltitle'])
                if filme_ou_serie == 'movie':
                    if comando == 'todos': progresso.update(int((float(current_page) / int(total_paginas) * 100)),
                                                            'A transferir biblioteca de Filmes...Aguarde...',
                                                            infolabels['originaltitle'] + ' (' + infolabels[
                                                                'Year'] + ')',
                                                            'Página ' + str(current_page) + '/' + str(total_paginas))
                    if not xbmcvfs.exists(os.path.join(moviesFolder, cleaned_title + ' (' + str(
                            infolabels["Year"]) + ')')): xbmcvfs.mkdir(
                        os.path.join(moviesFolder, cleaned_title + ' (' + str(infolabels["Year"]) + ')'))
                    strm_filme = os.path.join(moviesFolder, cleaned_title + ' (' + str(infolabels["Year"]) + ')',
                                              cleaned_title + '.strm')
                    strm_contents = 'plugin://plugin.video.ratotv/?url=' + url + '&mode=44&name=' + urllib.quote_plus(
                        infolabels['originaltitle'])
                    id_ratotv = re.compile('.*/(.+?)-.+?html').findall(url)[0]
            ...
            if comando == 'todos': progresso.update(int((float(current_page) / int(total_paginas) * 100)),
                                                    'A transferir biblioteca de Filmes...Aguarde...',
                                                    infolabels['originaltitle'] + ' (' + infolabels['Year'] + ')',
                                                    'Página ' + str(current_page) + '/' + str(total_paginas))
    ...
    if len(files) > 0:
        ...
        for ficheiro in files:
            ...
            if xbmcvfs.exists(txt_file):
                ...
                infolabels, name, url, iconimage, fanart, filme_ou_serie, HD, favorito = obter_info_url(serie_array[2],
                                                                                                        True)
                progresso.update(int(i / float(len(files)) * 100), 'A obter metadata...', name)
            addDir_filme(name + ' (' + str(infolabels["Year"]) + ')', url, 10, iconimage, infolabels, fanart,
                         len(files), True, filme_ou_serie, HD, favorito)
            i += 1
        progresso.close()
        moviesandseries_view()
    else:
...
def remover_subscricao_serie(name, url, iconimage):
    ...
    if yes:
        ...
        pass
    xbmc.executebuiltin(
        "XBMC.Notification(RatoTv,Removida subscrição com sucesso!,'10000'," + addonfolder + "/icon.png)")
    xbmc.executebuiltin("XBMC.Container.Refresh")


def subscrever_serie(name, url, iconimage, daemon=False):
    id_ratotv = re.compile('.*/(.+?)-.+?html').findall(url)[0]
    if not xbmcvfs.exists(os.path.join(datapath, 'tvshows-subscriptions')): xbmcvfs.mkdir(
        os.path.join(datapath, 'tvshows-subscriptions'))
    ...
    if selfAddon.getSetting('libraryfolder'):
        ...
        iconimage, originaltitle, year, serie_dict_temporadas = series_seasons_get_dictionary(url, name, "fanart")
        if not xbmcvfs.exists(os.path.join(tvshowsFolder, originaltitle)): xbmcvfs.mkdir(
            os.path.join(tvshowsFolder, originaltitle))
        total_seasons = len(serie_dict_temporadas.keys())
        for season in serie_dict_temporadas.keys():
            if not xbmcvfs.exists(os.path.join(tvshowsFolder, originaltitle, 'Season ' + str(season))): xbmcvfs.mkdir(
                os.path.join(tvshowsFolder, originaltitle, 'Season ' + str(season)))
            temp, year, episodios_dict = listar_temporadas_get_dictionary(" " + str(int(season)), url, "fanart",
                                                                          "iconimage", str(serie_dict_temporadas))
            for episode in episodios_dict.keys():
                ...
                save(os.path.join(tvshowsFolder, originaltitle, 'Season ' + str(season),
                                  originaltitle + ' ' + string + '.strm'), strm)
        if not daemon:
            xbmc.executebuiltin(
                "XBMC.Notification(RatoTv,Série subscrita com sucesso!,'10000'," + addonfolder + "/icon.png)")
            xbmc.executebuiltin("XBMC.UpdateLibrary(video," + os.path.join(tvshowsFolder, originaltitle) + ")")
    else:
        ok = mensagemok('RatoTV', 'Subscrição falhou!', 'Defina uma pasta de subscrições nas definições...')
    return
...
def play_from_outside(name, url):
    ...
    try:
        ...
        ok = mensagemok('RatoTV', 'Não conseguiu abrir o site!'), sys.exit(0)
    match = re.compile(
        '<img src="/templates/ratotvv2/dleimages/comments-img2.png"/><a href="http://ratotv.net/(.+?)/">.+?</a>').findall(
        html_source)
    if match[0] == 'tvshows':
        proceed = False
        ...
        if proceed:
            ...
            if proceed:
                proceed = False
                temp, year, episodios_dict = listar_temporadas_get_dictionary(" " + str(int(temporada)), url, "fanart",
                                                                              "iconimage", str(serie_dict_temporadas))
                for episode in episodios_dict.keys():
                ...
                if proceed:
                    screen = ''
                    if selfAddon.getSetting(
                            'series-episode-thumbnails') == 'true': screen = trakt_api().get_showepisode_thumb(
                        originaltitle, year, temporada, episodio)
                    if screen:
                        screenthumb = screen
                    else:
                        screenthumb = str(episodios_dict[str(int(episodio))]["thumbnail"])
                    episodios_opcao(str(episodios_dict[str(int(episodio))]["description"]), url, screenthumb,
                                    str(episodios_dict[str(int(episodio))]["source"]), originaltitle, temporada,
                                    episodio)
                else:
                    ok = mensagemok('RatoTV', 'Não encontrou o episódio')
    elif match[0] == 'movies':
        ...
        print 'Log: ocorreu um erro ou o item não é um filme nem uma série'
...
def listar_temporadas_get_dictionary(name, url, fanart, iconimage, dicionario):
    ...
    dic = eval(dicionario)
    episodes_dict = list_episodes(url, selfAddon.getSetting('login_name'), selfAddon.getSetting('login_password'),
...
def listar_temporadas(name, url, fanart, iconimage, dicionario):
    ...
    for episodio in sorted(episodios_dict.iterkeys(), key=keyfunc):
        ...
        if selfAddon.getSetting('series-episode-thumbnails') == 'true':
            ...
            screenimage = str(episodios_dict[episodio]["thumbnail"])
        addDir_episodio(name, "Episódio " + str(episodio),
                        urllib.unquote_plus(str(episodios_dict[episodio]["description"])), url, temporada, episodio,
                        str(episodios_dict[episodio]["source"]), screenimage, fanart,
                        episodios_dict[episodio]["watched"])
    episodes_view()
...
def addDir_temporada(name, url, dicionario, mode, iconimage, folder, fanart):
    u = sys.argv[0] + "?url=" + urllib.quote_plus(url) + "&mode=" + str(mode) + "&name=" + urllib.quote_plus(
        name) + "&dicionario=" + urllib.quote_plus(dicionario)
    try:
    ...
    if fanart: u += '&fanart=' + urllib.quote_plus(fanart)
    if originaltitle:
        u += "&originaltitle=" + urllib.quote_plus(originaltitle)
    ###AMELHORAR!
    else:
        try:
            html_source = post_page(url, selfAddon.getSetting('login_name'), selfAddon.getSetting('login_password'))
...
def addDir_filme(name, url, mode, iconimage, infolabels, fanart, totalit, pasta, tipo, HD, favorito, watched=None):
    u = sys.argv[0] + "?url=" + urllib.quote_plus(url) + "&mode=" + str(mode) + "&name=" + urllib.quote_plus(
        name) + "&iconimage=" + urllib.quote_plus(iconimage) + "&tipo=" + urllib.quote_plus(
        tipo) + "&year=" + urllib.quote_plus(str(year))
    try:
        u += "&year=" + infolabels['Year']
    except:
        pass
    try:
        u += "&imdb_id=" + infolabels['code']
    except:
        pass
    try:
        u += "&originaltitle=" + infolabels['originaltitle']
    except:
        pass
    try:
        id_ratotv = re.compile('.*/(.+?)-.+?html').findall(url)[0]
    ...
    playcount = 0
    contextmen = []
    if ADDON.getSetting('download-activo') == "true" and tipo == 'movie' and ADDON.getSetting(
            'folder') != "Escolha a pasta":
        contextmen.append(('Download', 'XBMC.RunPlugin(%s?mode=31&name=%s&url=%s&iconimage=%s&tipo=%s)' % (
            sys.argv[0], name, url, iconimage, urllib.quote_plus(tipo))))
    else:
        pass
    contextmen.append(('Estatísticas Trakt',
                       'XBMC.RunPlugin(%s?mode=30&name=%s&url=%s&iconimage=%s)' % (sys.argv[0], name, url, iconimage)))
    contextmen.append(('Ver detalhes', 'XBMC.Action(Info)'))
    contextmen.append(
        ('Classificar', 'XBMC.RunPlugin(%s?mode=19&name=%s&url=%s&iconimage=%s)' % (sys.argv[0], name, url, iconimage)))
    contextmen.append(('Reportar problema', 'XBMC.RunPlugin(%s?mode=24&url=%s&)' % (sys.argv[0], url)))
    if "Trailer" in (infolabels) and infolabels["Trailer"] != "": contextmen.append(
        ('Ver trailer', 'XBMC.PlayMedia(%s)' % (infolabels["Trailer"])))
    contextmen.append(('Ler Comentários',
                       'XBMC.RunPlugin(%s?mode=12&name=%s&url=%s&iconimage=%s)' % (sys.argv[0], name, url, iconimage)))
    contextmen.append(
        ('Comentar', 'XBMC.RunPlugin(%s?mode=7&name=%s&url=%s&iconimage=%s)' % (sys.argv[0], name, url, iconimage)))
    if tipo == 'movie':
        ...
        try:
            if trakt_instalado == True and selfAddon.getSetting("sync-trakt") == "true":
                ...
                for filme_visto in vistos:
                    if filme_visto["title"] == infolabels['originaltitle'] and str(filme_visto["year"]) == infolabels[
                        "Year"]: visto = True
        except:
            pass
        contextmen.append(('Adicionar à biblioteca', 'XBMC.RunPlugin(%s?mode=49&url=%s&name=%s&iconimage=%s)' % (
            sys.argv[0], url, name, iconimage)))
    else:
        visto = None
        if os.path.exists(filename):
            contextmen.append(('Deixar de seguir série', 'XBMC.RunPlugin(%s?mode=27&url=%s&name=%s&iconimage=%s)' % (
                sys.argv[0], url, name, iconimage)))
        else:
            contextmen.append(('Seguir série', 'XBMC.RunPlugin(%s?mode=25&url=%s&name=%s&iconimage=%s)' % (
                sys.argv[0], url, name, iconimage)))
        contextmen.append(('Próximo episódio?', 'XBMC.RunPlugin(%s?mode=28&url=%s&name=%s&iconimage=%s)' % (
            sys.argv[0], url, name, iconimage)))
        if not xbmcvfs.exists(os.path.join(datapath, 'tvshows-subscriptions', str(id_ratotv) + '.txt')):
            contextmen.append(('Subscrever série', 'XBMC.RunPlugin(%s?mode=43&url=%s&name=%s&iconimage=%s)' % (
                sys.argv[0], url, name, iconimage)))
        else:
            contextmen.append(('Remover subscrição', 'XBMC.RunPlugin(%s?mode=46&url=%s&name=%s&iconimage=%s)' % (
                sys.argv[0], url, name, iconimage)))
    if visto == True:
    ...
    if HD == True:
        ...
        liz.addStreamInfo('video', {'Codec': 'h264', 'width': 854, 'height': 480})
    elif HD == '3D':
        pass  # liz.setProperty('IsStereoscopic',True)
...
def addDir_episodio(nomeSerie, title, description, url, temporada, episodio, sources, thumbnail, fanart, watched):
    if description:
        ...
        episodeName = title
    u = sys.argv[0] + "?url=" + urllib.quote_plus(url) + "&mode=40" + "&iconimage=" + urllib.quote_plus(
        thumbnail) + "&seriesName=" + urllib.quote_plus(nomeSerie) + "&name=" + urllib.quote_plus(
        episodeName) + "&sources=" + urllib.quote_plus(str(sources)) + "&year=" + urllib.quote_plus(str(year))
    try:
        u += "&originaltitle=" + urllib.quote_plus(originaltitle)
    except:
        u += "&originaltitle=" + nomeSerie
    ...
    if ADDON.getSetting('download-activo') == "true" and ADDON.getSetting('folder') != "Escolha a pasta":
        contextmen.append(('Download',
                           'XBMC.RunPlugin(%s?mode=31&name=%s&url=%s&iconimage=%s&season=%s&episode=%s&originaltitle=%s&sources=%s)' % (
                               sys.argv[0], name, url, iconimage, temporada, episodio, nomeSerie,
                               urllib.quote_plus(str(sources)))))
    else:
        pass
    ...
    try:
        ...
        pass
    if visto:
        contextmen.append(('Marcar como não visto', 'XBMC.RunPlugin(%s?mode=22&url=%s&season=%s&episode=%s)' % (
            sys.argv[0], url, temporada, episodio)))
        overlay = 7
        playcount = 1
    else:
        contextmen.append(('Marcar como visto', 'XBMC.RunPlugin(%s?mode=21&url=%s&season=%s&episode=%s)' % (
            sys.argv[0], url, temporada, episodio)))
        overlay = 6
        playcount = 0
    if watched:
        contextmen.append(('Marcar como não visto (site)', 'XBMC.RunPlugin(%s?mode=58&url=%s&season=%s&episode=%s)' % (
            sys.argv[0], url, temporada, episodio)))
    else:
        contextmen.append(('Marcar como visto (site)', 'XBMC.RunPlugin(%s?mode=57&url=%s&season=%s&episode=%s)' % (
            sys.argv[0], url, temporada, episodio)))
    contextmen.append(('Ver detalhes', 'XBMC.Action(Info)'));
    contextmen.append(('Reportar problema', 'XBMC.RunPlugin(%s?mode=24&url=%s&)' % (sys.argv[0], url)))
    contextmen.append(('Estatísticas Trakt',
                       'XBMC.RunPlugin(%s?mode=30&name=%s&url=%s&iconimage=%s&season=%s&episode=%s)' % (
                           sys.argv[0], name, url, iconimage, temporada, episodio)))
    if not watched:
        if title != [] and description != []:
            titulo = '[COLOR white][B]' + title + '[/B][/COLOR]' + '-' + description
        elif title == [] and description != []:
            titulo = description
        elif title != [] and description == []:
            titulo = '[COLOR white][B]' + title + '[/B][/COLOR]'
        else:
            titulo = 'N/A'
    else:
        if title != [] and description != []:
            titulo = '[COLOR blue][B]' + title + '[/B]' + '-' + description + '[/COLOR]'
        elif title == [] and description != []:
            titulo = '[COLOR blue]' + description + '[/COLOR]'
        elif title != [] and description == []:
            titulo = '[COLOR blue]' + title + '[/COLOR]'
        else:
            titulo = '[COLOR blue]N/A[/COLOR]'
    liz = xbmcgui.ListItem(titulo, iconImage="DefaultFolder.png", thumbnailImage=thumbnail)
    if fanart: liz.setProperty('fanart_image', fanart)
    liz.setInfo(type="Video", infoLabels={"Title": title, "TVShowTitle": name, "Season": temporada, "Episode": episodio,
                                          "overlay": overlay, "playcount": playcount})
    maxRes = 720
    if maxRes == 1080:
        liz.addStreamInfo('video', {'Codec': 'h264', 'width': 1920, 'height': 1080})
    elif maxRes == 720:
        liz.addStreamInfo('video', {'Codec': 'h264', 'width': 1280, 'height': 720})
    elif maxRes == 480:
        liz.addStreamInfo('video', {'Codec': 'h264', 'width': 854, 'height': 480})
    else:
        liz.addStreamInfo('video', {'Codec': 'h264', 'width': 640, 'height': 360})
    liz.addContextMenuItems(contextmen, replaceItems=True)
...
try:
    ...
    pass
try:
    tipo = urllib.unquote_plus(params["tipo"])
except:
    pass
try:
    infolabels_trailer = urllib.unquote_plus(params["infolabels_trailer"])
except:
    pass
try:
...
try:
    ...
    pass
try:
    originaltitle = urllib.unquote_plus(params["originaltitle"])
except:
    pass
try:
    sources = urllib.unquote_plus(params["sources"])
...
try:
    ...
    pass
try:
    dicionario = urllib.unquote_plus(params["dicionario"])
except:
    pass
try:
...
if mode == None or url == None or len(url) < 1:
    ...
    Menu_categorias_filmes()
elif mode == 6:
    filmes_homepage(name, url)
elif mode == 7:
    comment(url)
elif mode == 8:
    Menu_principal_series()
elif mode == 81:
    Menu_principal_animes()
elif mode == 9:
    alterar_definicoes()
elif mode == 10:
    series_seasons(url, name, fanart)
elif mode == 11:
    season_episodes_um(url, name, season, fanart)
elif mode == 12:
    ler_comentarios(url, '')
elif mode == 13:
    pedir_serie_menu()
elif mode == 14:
    add_to_favourites(url)
elif mode == 15:
    listar_favoritos(url)
elif mode == 16:
    listar_pesquisa(url)
elif mode == 17:
    remover_favoritos(url)
elif mode == 18:
    play_trailer(infolabels_trailer)
elif mode == 19:
    votar_ratotv()
elif mode == 21:
    adicionar_visto(url, season, episode)
elif mode == 22:
    remover_visto(url, season, episode)
elif mode == 23:
    pass
elif mode == 24:
    reportar(url)
elif mode == 25:
    adicionar_seguir(url, name, iconimage)
elif mode == 26:
    listar_series_subseguir("seguir")
elif mode == 27:
    deixar_seguir(url)
elif mode == 28:
    proximo_episodio(url)
elif mode == 29:
    pass
elif mode == 30:
    estatisticas_trakt(url)
elif mode == 31:
    download_qualidade(url, name, iconimage)
elif mode == 32:
    season_episodes_dois(url, name, season, fanart)
elif mode == 33:
    menu_pedidos(url)
elif mode == 34:
    pedir_id(url)
    ...
    listar_temporadas(name, url, fanart, iconimage, dicionario)
elif mode == 40:
    episodios_opcao(name, url, iconimage, sources, originaltitle, season, episode)
elif mode == 41:
    pesquisa_ano(url)
elif mode == 42:
    menu_ano()
elif mode == 43:
    subscrever_serie(name, url, iconimage)
elif mode == 44:
    play_from_outside(name, url)
elif mode == 45:
    listar_series_subseguir("subscritas")
elif mode == 46:
    remover_subscricao_serie(name, url, iconimage)
elif mode == 47:
    transferir_biblioteca_filmes(name)
elif mode == 48:
    procurar_novas_series(name)
elif mode == 49:
    adicionar_filme_biblioteca(name, url, iconimage)
elif mode == 50:
    trending_menu_trakt()
elif mode == 51:
    filmes_trending()
    ...
    filmes_watchlist(name)
elif mode == 54:
    series_watchlist(name)
elif mode == 55:
    filmes_collection_trakt(name)
elif mode == 56:
    series_collection_trakt(name)
elif mode == 57:
    adicionar_rato_visto(url, season, episode)
elif mode == 58:
    remover_rato_visto(url, season, episode)
elif mode == 59:
    list_wachlist(url, mode)
elif mode == 100:
    transferir_biblioteca_filmes(name, 'favorites')
